...
class DebouncedInput {
	int m_state;   // the current reading from the input pin
	int m_lastState;   // the previous reading from the input pin
	int m_input;
...
public:
	...
	DebouncedInput(const int i_inputPin):m_input(i_inputPin),m_lastDebounceTime(0),m_debounceDelay(50) {
		m_state = m_lastState = digitalRead( m_input );
	}
	...
	int read() {
		...
		// (i.e. the input went from LOW to HIGH), and you've waited long enough
		// since the last press to ignore any noise:
		int reading = digitalRead( m_input );
...
public:
	...
	PushButton( int i_input, unsigned i_debounceDelay = 50,
			...
			unsigned long i_multiclick_threshold= 400 ):
				DebouncedInput(i_input), m_btn_state(sUP), m_clicks(0),
...
class MorseCoder {
	...
	unsigned      m_subtick;  ///< ticks for this symbol left
	const char*   m_msg;      ///<
	unsigned      m_len;
...
//		Serial.printf(" is active: %s\n", m_msg ? "TRUE" : "false");
	...
	static const MorseSymbol long_space;
};
...
#define SEEK_EEPROM( pos )eeprom_wp=(pos);
